...
if (!function_exists('theme_get_metadata_icons')){
	function theme_get_metadata_icons($icons = '', $class=''){
		...
		for($i = 0; $i < count($icons); $i++){
			...
			switch($icon){
				case 'date':
					$result[] = theme_get_meta_icon('postdateicon', 13, 15) . sprintf( __('<span class="%1$s">Published</span> %2$s', THEME_NS),
									'date',
				...
				break;
				case 'author':
					$result[] = theme_get_meta_icon('postauthoricon', 18, 18) . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
									'author',
				...
				case 'category':
					...
					$result[] = theme_get_meta_icon('postcategoryicon', 18, 18) . sprintf(__('<span class="%1$s">Posted in</span> %2$s', THEME_NS), 'categories', get_the_category_list(', '));
				break;
				case 'tag':
				...
				case 'comments':
					...
					$result[] = theme_get_meta_icon('postcommentsicon', 18, 18) . ob_get_clean();
				break;
				case 'edit':
					if (!current_user_can('edit_post', $post->ID)) break;
					ob_start();
					edit_post_link(__('Edit', THEME_NS), '');
					$result[] = theme_get_meta_icon('postediticon', 13, 13) . ob_get_clean();
				break;
			}
		}
